...
JSON;
    ...
    public function testErrorWhenContentTypeIsInvalid()
    {
        $client = $this->createAuthenticatedClient([], ['CONTENT_TYPE' => 'application/json']);
...
{
    "code": 415,
    "message": "\"application\/json\" in \"Content-Type\" header is not valid. Only \"multipart\/form-data\" is allowed."
}
JSON;
        $this->assertJsonStringEqualsJsonString($expectedResponse, $response->getContent());
    }

    public function testErrorWhenContentTypeIsBlank()
    {
        $client = $this->createAuthenticatedClient([], ['CONTENT_TYPE' => '']);
...
{
    "code": 415,
    "message": "\"\" in \"Content-Type\" header is not valid. Only \"multipart\/form-data\" is allowed."
}
JSON;
        $this->assertJsonStringEqualsJsonString($expectedResponse, $response->getContent());
    }

    public function testHeaderAcceptWithInvalidValueIsIgnored()
    {
        $client = $this->createAuthenticatedClient([], ['CONTENT_TYPE' => 'multipart/form-data']);
        ...
        $client->request('POST', '/api/rest/v1/media-files', $content, ['file' => $file], ['HTTP_ACCEPT' => 'accept_unknown']);
        $response = $client->getResponse();

        $this->assertSame(Response::HTTP_CREATED, $response->getStatusCode());
    }

    public function testHeaderAcceptWithBlankValueIsIgnored()
    {
        $client = $this->createAuthenticatedClient([], ['CONTENT_TYPE' => 'multipart/form-data']);
        ...
        $client->request('POST', '/api/rest/v1/media-files', $content, ['file' => $file], ['HTTP_ACCEPT' => '']);
        $response = $client->getResponse();
